...
public class AuthEventHandler {
    ...
    public static LiteralText checkCanPlayerJoinServer(GameProfile profile, PlayerManager manager) {
        ...
        String regex = EasyAuth.config.main.usernameRegex;
        ...
        if((onlinePlayer != null && !((PlayerAuth) onlinePlayer).canSkipAuth()) && EasyAuth.config.experimental.preventAnotherLocationKick) {
            ...
            return new LiteralText(
                    String.format(
                            EasyAuth.config.lang.playerAlreadyOnline, onlinePlayer.getName().asString()
                    )
            );
        }
            ...
            return new LiteralText(
                    String.format(
                            EasyAuth.config.lang.disallowedUsername, regex
                    )
            );
        }
        return null;
    ...
    public static void onPlayerJoin(ServerPlayerEntity player) {
        ...
        if(EasyAuth.config.main.tryPortalRescue && player.getBlockStateAtPos().getBlock().equals(Blocks.NETHER_PORTAL)) {
    ...
    public static void onPlayerLeave(ServerPlayerEntity player) {
        ...
        if(playerCache.isAuthenticated) {
            ...
            if(EasyAuth.config.main.sessionTimeoutTime != -1)
                playerCache.validUntil = System.currentTimeMillis() + EasyAuth.config.main.sessionTimeoutTime * 1000L;
        }
    ...
    public static ActionResult onPlayerChat(ServerPlayerEntity player, String message) {
        ...
        if(
            ...
            (!EasyAuth.config.experimental.allowChat || message.startsWith("/"))
        ) {
