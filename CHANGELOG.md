...
- Automatic two-factor codes: store an account's TOTP secret in the server entry and the client computes and fills the current 6-digit code on auto-login (packet path and the `{otp}` placeholder)
  - Optional separate 2FA command template for servers whose login command takes no code — set it to `/totp code {otp}` for AuthMeReloaded and it is sent right after the login command
- Config screen via ModMenu (Fabric) or the mods-list Config button (NeoForge): per-server passwords and toggles, global settings, command templates, and a "Forget session token & passkey" button per server; passwords are displayed masked (with a Show/Hide toggle in the editor)
- A "…" button next to the address box of the vanilla edit-server screen (Multiplayer → Edit) opens the EasyAuth settings for that server directly
- Stored secrets (passwords, TOTP secrets, session tokens, passkey private keys) are encrypted at rest with AES-256-GCM; on Windows the key file is additionally protected with DPAPI. An optional master password (asked once per launch) can protect it on any platform, or encryption can be turned off entirely to keep everything as plain text. By default the data and the key file live in the instance's `config/easyauth-client` folder; the "Storage & security" screen has one-click home-folder buttons to share one credential store between instances and to keep the key outside an exported or synced instance. Existing plaintext files are migrated automatically
- Auto-input rules in `config/easyauth-client/rules.json`: send chat messages or commands on join, on a chat-message regex, on a timer, or on leave (the pause-menu Disconnect button), with `{username}`, `{server}`, `{password}`, `{otp}` placeholders, delays, cooldowns and per-session run limits
  - Rules can be added, edited and removed in-game from the "Auto-input rules" screen in the config screen, so the file no longer has to be hand-edited
